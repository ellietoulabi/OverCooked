...
class OvercookedEnv(object):
    ...
    def run_agents(
        ...
        while not done:
            ...
            #El
            for agent_idx, agent in enumerate(agent_pair.agents):
                if isinstance(agent, QLearningAgent):
                    agent.update({
                        'prev_state': s_t,
                        'next_state': s_tp1,
                        'total_sparse_reward': r_t,
                        'done': done,
                        'info': info,
                        'actions': a_t,
                        'sparse_r_by_agent': info['sparse_r_by_agent'],
                        'shaped_r_by_agent': info['shaped_r_by_agent'],
                    })
            #El
            # agent_pair.notify_agents((s_tp1, s_t, a_t, r_t, done, info))
